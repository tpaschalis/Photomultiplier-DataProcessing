...
def AdditionalBranches():
	...
	# for i in xrange(200):
	for i in xrange(events_in_file-2):
		if i%1000==0:
	...
	print "Time elapsed in the AdditionalBranches func = "+ str(time.time() - t)	
		# print "len(rms_allevents)", len(rms_allevents)
		# print rms_allevents
		# print "len(rms_allevents[1])". len(rms_allevents[0])
		# print "len(rms_allevents[1])", len(rms_allevents[1])
...
rms=[]
for x in xrange (points_per_event):
	rms.append(math.sqrt(meansquared[x]-mean[x]**2))

meanerror=[]
for x in xrange (points_per_event):
	meanerror.append(math.sqrt(meansquared[x]-mean[x]**2)/math.sqrt(events_in_file))
...
for x in xrange(events_in_file):
	dict[x]=[]
	for u in xrange (points_per_event):
		dict[x].append(rundata[points_per_event*x+u])

# We use this to make the x'x axis
dict[events_in_file]=[]
for i in xrange(points_per_event):
	dict[0][i]=i											# Multiply this line by time factor

	
# print type(dict)							# dict
# print type(dict[1])						# list
# print type(dict[4][3])					# float

# Stuff gets written and read in the same order
# for i in xrange(50):
for i in xrange(events_in_file-2):					# Use this line for full parsing.
